/**
 * Side-pane Today view. Renders today's Dayflow note inline using
 * MarkdownRenderer so the user gets the same callout + timeline they'd
 * see by opening the note, but in a sidebar that refreshes after every
 * sync.
 *
 * Why MarkdownRenderer instead of custom DOM? The note already has the
 * exact structure we'd want (callout, timeline cards, etc.). Re-rendering
 * the markdown keeps the two views in lockstep with zero duplication.
 */
...
import { getDayString } from '../boundary.js';

export const TODAY_VIEW_TYPE = 'dayflow-today-view';
...
export class TodayView extends ItemView {
  ...
  /** Re-read today's Dayflow note and render it inline. */
  async refresh(): Promise<void> {
    ...
    const path = `${this.plugin.settings.outputFolder.replace(/\/+$/, '')}/${this.plugin.settings.dailySubfolder}/Dayflow_${today}.md`;
    ...
    try {
      ...
      // Strip leading frontmatter so the rendered view starts at the H1.
      ...
      new Notice('Dayflow: failed to render today view');
    }
  }
}
